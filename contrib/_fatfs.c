...
static
FDIR *fatfs_dir_alloc(void);

static
void fatfs_dir_free(FDIR *fp);
...
static
void fill_fd_dir(int fildes, FDIR *fp, int flags, const FILINFO *fno);
...
/* typedef struct dirstream FDIR in dirent.h */
...
struct dirent *fatfs_readdir(FDIR *dirp)
{
    struct dirent *ret;
...
void fatfs_rewinddir(FDIR *dirp)
{
    if (!is_dir(dirp))
    {
...
void fatfs_seekdir(FDIR *dirp, long loc)
{
    if (is_dir(dirp))
    {
        long cur_loc;
